...
import type { Quotation, QuotationItem, Invoice, Contract, Report, User, Project } from '@/lib/db/types';
...
export async function generateContractPdf(
  ...
  const items: PdfLineItem[] = [
    { description: contract.type || 'Service Agreement', quantity: '1', unit_price: '—', total: '—' },
  ];
  ...
  return generateDocument(
    {
      ...
      items,
      totals: [{ label: 'Status', value: contract.status, bold: true }],
      notes: `Contract Type: ${contract.type}\nParties: ${contract.parties}\nExpiry: ${formatDate(contract.expiry, settings)}\nProgress: ${contract.progress}%`,
      terms: settings.default_terms || undefined,
    },
    'CONTRACT',
    `Contract-${contract.title.replace(/\s+/g, '_')}.pdf`,
    style,
  );
}
...
export async function generateReportPdf(
  ...
  const items: PdfLineItem[] = [
    { description: report.type || 'Report', quantity: '1', unit_price: '—', total: '—' },
  ];
  ...
  return generateDocument(
    {
      ...
      items,
      totals: [{ label: 'Recipients', value: report.recipientCount.toString(), bold: false }],
      notes: `Type: ${report.type}\nFormat: ${report.format}\nNext Run: ${formatDate(report.nextRun, settings)}`,
    },
    'REPORT',
    `Report-${report.name.replace(/\s+/g, '_')}.pdf`,
    style,
  );
}
